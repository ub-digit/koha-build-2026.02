...
sub GetImportRecordMarc {
    ...
    my $dbh = C4::Context->dbh;
    my ( $marc, $encoding ) = $dbh->selectrow_array(
        q|
...
sub AddImportBatch {
    ...
    my $dbh = C4::Context->dbh;
    $dbh->do(
        "INSERT INTO import_batches (" . join( ',', @fields ) . ")
...
sub GetImportBatch {
    ...
    my $sth = $dbh->prepare_cached(
        "SELECT b.*, p.name as profile FROM import_batches b LEFT JOIN import_batch_profiles p ON p.id = b.profile_id WHERE import_batch_id = ?"
    );
...
sub AddBiblioToBatch {
    ...
    my $import_record_id = _create_import_record(
        $batch_id, $record_sequence, $marc_record, 'biblio', $encoding,
...
sub AddAuthToBatch {
    ...
    my $import_record_id =
        _create_import_record( $batch_id, $record_sequence, $marc_record, 'auth', $encoding, $marc_type );
...
sub BatchFindDuplicates {
    ...
    my $sth = $dbh->prepare(
        "SELECT import_record_id, record_type, marc
                             FROM import_records
...
sub BatchCommitRecords {
    ...
    my $dbh = C4::Context->dbh;
    my $sth = $dbh->prepare(
        "SELECT import_records.import_record_id, record_type, status, overlay_status, marc, encoding
                             FROM import_records
    ...
    while ( my $rowref = $sth->fetchrow_hashref ) {
        ...
        my ( $record_result, $item_result, $record_match ) = _get_commit_action(
        ...
        if ( $record_result eq 'create_new' ) {
            ...
            if ( $record_type eq 'biblio' ) {
                ...
                if ( $item_result eq 'create_new' || $item_result eq 'replace' ) {
                    my ( $bib_items_added, $bib_items_replaced, $bib_items_errored ) =
                        _batchCommitItems( $rowref->{'import_record_id'}, $recordid, $item_result, $biblioitemnumber );
            ...
            if ( $record_type eq 'biblio' ) {
                ...
                my $old_marc =
                    MARC::Record->new_from_xml( StripNonXmlChars($oldxml), 'UTF-8', $rowref->{'encoding'}, $marc_type );
                ...
                if ( $item_result eq 'create_new' || $item_result eq 'replace' ) {
                    my ( $bib_items_added, $bib_items_replaced, $bib_items_errored ) =
                        _batchCommitItems( $rowref->{'import_record_id'}, $recordid, $item_result );
                    $num_items_added    += $bib_items_added;
                    $num_items_replaced += $bib_items_replaced;
                    $num_items_errored  += $bib_items_errored;
                }
            ...
            my $sth = $dbh->prepare_cached(
                "UPDATE import_records SET marcxml_old = ?, status = ?, overlay_status = ? WHERE import_record_id = ?");
            ...
            $num_ignored++;
            if (    $record_type eq 'biblio'
                and defined $recordid
                and ( $item_result eq 'create_new' || $item_result eq 'replace' ) )
            {
                my ( $bib_items_added, $bib_items_replaced, $bib_items_errored ) =
                    _batchCommitItems( $rowref->{'import_record_id'}, $recordid, $item_result );
...
sub _batchCommitItems {
    ...
    while ( my $row = $sth->fetchrow_hashref() ) {
        ...
        my $item =
            TransformMarcToKoha( { record => $item_marc, kohafields => [ 'items.barcode', 'items.itemnumber' ] } );
        ...
        my $updsth = $dbh->prepare(
            "UPDATE import_items SET status = ?, itemnumber = ?, import_error = ? WHERE import_items_id = ?");
...
sub BatchRevertRecords {
    ...
    my $dbh            = C4::Context->dbh;
    my $sth            = $dbh->prepare(
        "SELECT import_records.import_record_id, record_type, status, overlay_status, marcxml_old, encoding, matched_biblionumber, matched_authid
                             FROM import_records
                             LEFT JOIN import_auths ON (import_records.import_record_id=import_auths.import_record_id)
                             LEFT JOIN import_biblios ON (import_records.import_record_id=import_biblios.import_record_id)
                             WHERE import_batch_id = ?"
    ...
    while ( my $rowref = $sth->fetchrow_hashref ) {
        ...
        if ( $record_result eq 'delete' ) {
            ...
            if ( $record_type eq 'biblio' ) {
                $num_items_deleted +=
                    BatchRevertItems( $rowref->{'import_record_id'}, $rowref->{'matched_biblionumber'} );
            ...
            $num_reverted++;
            my $old_record = MARC::Record->new_from_xml(
                StripNonXmlChars( $rowref->{'marcxml_old'} ), 'UTF-8',
            ...
            if ( $record_type eq 'biblio' ) {
                ...
                $logger->info(
                    "C4::ImportBatch::BatchRevertRecords: Biblio record $biblionumber does not exist, restoration of this record was skipped"
                ) unless $oldbiblio;
        ...
        if ( $record_type eq 'biblio' ) {
            ...
            $query =
                "UPDATE import_biblios SET matched_biblionumber = NULL WHERE import_record_id = ?";    # FIXME Remove me
            $query =
                "UPDATE import_biblios SET matched_biblionumber = NULL WHERE import_record_id = ?  AND NOT EXISTS (SELECT * FROM import_items WHERE import_items.import_record_id=import_biblios.import_record_id and status='imported')";
        } else {
...
sub GetImportRecordsRange {
    ...
    ($order_by) =
        grep( { $_ eq $order_by } qw( import_record_id title status overlay_status ) ) ? $order_by : 'import_record_id';

    my $order_by_direction = uc( $parameters->{order_by_direction} // 'ASC' ) eq 'DESC' ? 'DESC' : 'ASC';
...
sub _create_import_record {
    ...
    my $dbh = C4::Context->dbh;
    my $sth = $dbh->prepare(
        "INSERT INTO import_records (import_batch_id, record_sequence, marc, marcxml, marcxml_old,
                                                         record_type, encoding)
                                    VALUES (?, ?, ?, ?, ?, ?, ?)"
    );
    $sth->execute(
        $batch_id,    $record_sequence, $marc_record->as_usmarc(), $marc_record->as_xml($marc_type), '',
        $record_type, $encoding
    );
...
sub _add_auth_fields {
    ...
    my $dbh                = C4::Context->dbh;
    my $sth                = $dbh->prepare(
        "INSERT INTO import_auths (import_record_id, control_number, authorized_heading) VALUES (?, ?, ?)");
...
sub _add_biblio_fields {
    ...
    my $sth = $dbh->prepare(
        "INSERT INTO import_biblios (import_record_id, title, author, isbn, issn) VALUES (?, ?, ?, ?, ?)");
...
sub _parse_biblio_fields {
    ...
    my $dbh          = C4::Context->dbh;
    my $bibliofields = TransformMarcToKoha(
        {
            record     => $marc_record,
            kohafields => [ 'biblio.title', 'biblio.author', 'biblioitems.isbn', 'biblioitems.issn' ]
        }
    );
...
sub _update_batch_record_counts {
    ...
    my $dbh = C4::Context->dbh;
    my ($num_records) = $dbh->selectrow_array(
        q|
                                            SELECT COUNT(*)
                                            FROM import_records
                                            WHERE import_batch_id = ?
    ...
    my ($num_items) = $dbh->selectrow_array(
        q|
                                            SELECT COUNT(*)
                                            FROM import_records
...
sub _get_commit_action {
    ...
    if ( $record_type eq 'biblio' ) {
        ...
        if ( $overlay_status ne 'no_match' && defined($bib_match) ) {
            ...
            $item_result =
                ( $item_action eq 'always_add' or $item_action eq 'add_only_for_new' ) ? 'create_new' : 'ignore';
        }
