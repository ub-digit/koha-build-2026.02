...
my ( $template, $loggedinuser, $cookie ) = get_template_and_user(
    {
        template_name => "tools/manage-marc-import.tt",
        query         => $input,
        type          => "intranet",
...
if ( $op eq "create_labels" ) {
    ...
    if ( $label_batch_id == -1 ) {
        $template->param(
            label_batch_msg => "error",
            message_type    => 'alert',
        );
    } else {
        $template->param(
            label_batch_msg => $label_batch_id,
            message_type    => 'dialog',
        );
    }
    $op              = '';
...
if ( $op eq "" ) {
    ...
    try {
        my $job_id =
            Koha::BackgroundJob::MARCImportRevertBatch->new->enqueue( { import_batch_id => $import_batch_id } );
        if ($job_id) {
            $template->param(
                job_enqueued => 1,
                job_id       => $job_id,
...
sub redo_matching {
    my (
        $template, $import_batch_id, $new_matcher_id, $current_matcher_id, $overlay_action, $nomatch_action,
        $item_action
    ) = @_;
    my $rematch_failed = 0;
...
sub create_labelbatch_from_importbatch {
    ...
    if ( grep { $_ == 0 } @items ) {
        warn sprintf(
            'create_labelbatch_from_importbatch() : Call to C4::ImportBatch::GetItemNumbersFromImportBatch returned no item number(s) from import batch #%s.',
            $batch_id
        );
        return -1;
    }
    foreach my $item_number (@items) {
        ...
        if ( $err == -1 ) {
            warn sprintf(
                'create_labelbatch_from_importbatch() : Error attempting to add item #%s of import batch #%s to label batch.',
                $item_number, $batch_id
            );
            return -1;
        }
    }
